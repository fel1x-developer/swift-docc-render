...
function addENVDefaults() {
  if (typeof import.meta.env.VITE_APP_TITLE === 'undefined') {
    import.meta.env.VITE_APP_TITLE = 'Documentation';
  }
}
...
function baseChainWebpack(config) {
  ...
  const envLanguages = (import.meta.env.VITE_APP_HLJS_LANGUAGES ?? '').split(',').join('|');
  config
...
function baseDevServer({ defaultDevServerProxy = 'http://localhost:8000' } = {}) {
  const devServerProxy = import.meta.env.VITE_APP_DEV_SERVER_PROXY || defaultDevServerProxy;
  ...
  return localFixtures ? ({
    // If `$VITE_APP_DEV_SERVER_PROXY` is a path that exists on the local
    ...
    // Otherwise, use the `$VITE_APP_DEV_SERVER_PROXY` value as an http endpoint
