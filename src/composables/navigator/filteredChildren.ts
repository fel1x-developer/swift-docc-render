...
export default function filterChildren(nodesToRender) {
  const navigatorItems = computed(() => nodesToRender);

  /**
   * Returns a list of the child nodes, that match the filter pattern.
   * @returns NavigatorFlatItem[]
   */
  function filterChildren(children, selectedTags, apiChanges, filterPattern) {
  ...
  return { navigatorItems, filterChildren };
}
